...
namespace AdvancedGraphics {
	...
	public struct PDFRect {
		...
		public static PDFRect Infinity {
			get {
				return new PDFRect(double.NegativeInfinity, double.NegativeInfinity,
	...
	[StructLayout(LayoutKind.Auto)]
	public struct PDFFormXObjectID {
		public int ID;
	...
	public class PDFBoundedContent: PDFContent {
		...
		public PDFBoundedContent(Doc doc) : this(doc,
			doc.AddObject("<</Type /XObject /Subtype /Form"
			+" /Resources <</ProcSet[/PDF /Text /ImageB /ImageC /ImageI]>>>>stream\nendstream\n"))
		{ }

		public PDFBoundedContent(Doc doc, int xobjID) : base(doc) {
			_xobjID = xobjID;
		...
		public PDFFormXObjectID WriteToFormXObject() {
			...
			mDoc.SetInfo(_xobjID, "stream", mContents.ToString());
			mDoc.SetInfo(_xobjID, "/BBox:Rect", _bounds.ToString());
			ShowErrors();
		...
		public override void SetTextRenderingMode(PDFContent.TextRenderingMode mode) {
		...
		public override void ShowTextString(string text) {
			...
			string s = mDoc.GetInfo(_state.FontID, "Widths "+text);
			if(s.Length<=0)
				return;

			if(_fontInfoID!=_state.FontID) {
				_fontIsMultibyte = mDoc.GetInfo(_state.FontID, "/Subtype:Name")=="Type0";
				_fontBBox.String = mDoc.GetInfo(_state.FontID, "FontBBox");
